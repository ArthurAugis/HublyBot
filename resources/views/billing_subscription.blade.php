        ...
        <div class="flex-grow p-6 lg:p-10 max-w-4xl w-full mx-auto space-y-6 pt-20 lg:pt-10">
            ...
            <div class="cyber-card rounded-2xl p-6 md:p-8 space-y-6">
                <h3 class="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                    <i data-lucide="sparkles" class="w-4 h-4 text-[#5865F2]"></i> Active Plan Details
                </h3>
                ...
                <div class="p-5 rounded-2xl bg-white/5 border border-white/10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    ...
                    <div>
                        @if ($activeOrder && (!isset($cancelAtPeriodEnd) || !$cancelAtPeriodEnd))
                            <form action="{{ route('subscription.cancel') }}" method="POST" onsubmit="showConfirmationModal(event, this, 'Cancel Subscription', 'Are you sure you want to cancel your subscription? Your bot hosting will continue until the end of the current billing cycle, after which your bots will be paused.', 'Cancel Subscription', true);" class="m-0">
                                @csrf
                                <button type="submit" class="px-4 py-2.5 rounded-xl border border-rose-500/20 bg-rose-500/5 hover:bg-rose-500/10 text-xs font-bold text-rose-400 transition cursor-pointer">
                                    Cancel Subscription
                                </button>
                            </form>
                        @elseif (!$activeOrder)
                            <a href="{{ route('pricing') }}" class="inline-flex neon-glow-btn text-white text-xs font-bold px-5 py-2.5 rounded-xl gap-1.5 cursor-pointer">
                                Upgrade Plan <i data-lucide="arrow-right" class="w-4 h-4"></i>
                            </a>
                        @endif
                    </div>
                </div>
